...
export class SwapAction {
    private lifiConfig;
    private bebopChainsMap: { [chain: string]: string } = {
        celo: "celo",
        alfajores: "alfajores",
    };
    ...
    constructor(private walletProvider: WalletProvider) {
        ...
        for (const config of Object.values(this.walletProvider.chains)) {
            if (!["celo", "alfajores"].includes(config.name.toLowerCase())) {
                continue;
            }
            try {
                lifiChains.push({
                    ...
                    mainnet: config.name.toLowerCase() === "celo",
                    diamondAddress:
    ...
    private async getBebopQuote(
        ...
        try {
            const chainKey =
                this.bebopChainsMap[params.chain.toLowerCase()] ?? params.chain;
            const url = `https://api.bebop.xyz/router/${chainKey}/v1/quote`;
...
export const swapAction = {
    name: "swap",
    description: "Swap tokens on the Celo or Alfajores network",
    handler: async (
        ...
        [
            {
                user: "user",
                content: {
                    text: "Swap 1 CELO for cUSD on Celo",
                    action: "TOKEN_SWAP",
                },
            },
        ],
